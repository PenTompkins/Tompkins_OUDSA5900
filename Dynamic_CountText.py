# -*- coding: utf-8 -*-
#This file is being created to perform 'Dynamic_Count' process over truly textual tweets only
#Thus, all tweets containing links or other forms of media attachments will be removed from examination
#However, tweets containing only emojis will still be included in analysis
#!Note: Due to time constraints, this file ultimately has not been used in any analysis
#
#Overall, this is program: 29
...
#For this version, remove tweets containing links from consideration. Examine truly textual tweets only
linkPat = "http"
linkFilter = company_tweets["Content"].str.contains(linkPat)
company_tweets = company_tweets[~linkFilter]
company_tweets2 = company_tweets2[~linkFilter]
print(company_tweets.shape)
...
#Adding in line to replace extra whitespace with single whitespace
...
#Removing tweets that weren't originally in English, or UND (consisting of emojis)
...
#Removing rows with no text left inside them...NOT ANYMORE
#filter1 = English_tweets["Content"] != ""
#cleanGlish_tweets = English_tweets[filter1]
cleanGlish_tweets = pd.concat([English_tweets, und_tweets], axis=0)
...
#In the case that there aren't enough official tweets for analysis:
    ...
    ##Perform same analysis over median 50% of all tweets: (in this case, mostly IRT)
...
#In case there aren't enough IRT tweets for analysis:
    ...
    #Grab the bottom 25% performing tweets
    bottom_official = official_clean.tail(nr).copy()
    #print(bottom_official.shape)
