#This file is being created to run sentiment effect val. process over Mercedes OT tweets
#
...
#Since the log dists. are all normal, try ANOVA
nova = aov(logLikes ~ sent_cat, data = tweets)
summary(nova)
...
ggdensity(pos_tweets$logRetweets,
          ...
          xlab = "log(Retweets)") #+ xlim(min(pos_tweets$logRetweets), max(pos_tweets$logRetweets))
