#This program is being created to generate KNN predictions for Amazon OT data
#
#Overall, this is program: 90
...
grid = expand.grid(kmax = 20:50,
                   distance = 1:5,
                   kernel = c('gaussian', 'triangular', 'rectangular', 'epanechnikov', 'optimal', 'rank'))
...
model_ = train(`Number of Likes` ~ topic.1 + topic.2 + topic.3 + topic.4 + topic.5 +
                 ...
                 topic.13 + topic.14 + topic.15 + topic.16, data = train_data, method = "kknn",
               trControl = control, tuneGrid = grid)
print(model_)
...
write_xlsx(df, "C:\\Users\\Pendl\\OneDrive\\Documents\\TwitterProject\\ModelingResults_OT\\Storage\\KNN_Amazon_OT.xlsx")
write_xlsx(df2, "C:\\Users\\Pendl\\OneDrive\\Documents\\TwitterProject\\ModelingResults_OT\\Storage\\KNN_Amazon_OTparams.xlsx")
