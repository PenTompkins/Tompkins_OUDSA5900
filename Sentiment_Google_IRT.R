#This file is being created to run sentiment effect val. process over Google IRT tweets
#
...
data = read_excel("C:/Users/Pendl/OneDrive/Documents/TwitterProject/OutlierFree_data/withSentiment/Google_outsRem_ws.xlsx")

#Extract IRT tweets only:
tweets = data[which(data$IRT == TRUE),]
...
#Remove 'negative' tweets from data, as there are only 4:
tweets = tweets[-which(tweets$sent_cat == "Negative"),]
...
ggdensity(pos_tweets$`Number of Likes`,
          main = "Google Positive: Likes",
          xlab = "Number of Likes") + xlim(0, max(neut_tweets$`Number of Likes`))

#Visualize dist. of likes for Neutral tweets:
ggdensity(neut_tweets$`Number of Likes`,
          main = "Google Neutral: Likes",
...
ggdensity(pos_tweets$logLikes,
          main = "Google Positive: log(Likes)",
...
#          main = "Google Negative: log(Likes)",
#          xlab = "log(Likes)") + xlim(min(neg_tweets$logLikes), max(pos_tweets$logLikes))
...
#Perform Mann Whitney U test on neutral vs positive tweets (for likes):
res = wilcox.test(neut_tweets$`Number of Likes`, pos_tweets$`Number of Likes`)
res
...
df = mkt2(neut_tweets$`Number of Likes`, pos_tweets$`Number of Likes`)
...
df = mkt2(neut_tweets$logLikes, pos_tweets$logLikes)
sf = shifthd_pbci(data = df, formula = obs ~gr, doall = TRUE)
...
ggdensity(neut_tweets$`Number of Retweets`,
          main = "Google Neutral: Retweets",
          xlab = "Number of Retweets") + xlim(0, max(pos_tweets$`Number of Retweets`))

#Visualize dist. of Retweets for Negative tweets:
#ggdensity(neg_tweets$`Number of Retweets`,
#          main = "Google Negative: Retweets",
#          xlab = "Number of Retweets") + xlim(0, max(pos_tweets$`Number of Retweets`))
...
ggdensity(pos_tweets$logRetweets,
          main = "Google Positive: log(Retweets)",
...
ggdensity(neut_tweets$logRetweets,
          main = "Google Neutral: log(Retweets)",
          xlab = "log(Retweets)") + xlim(min(neut_tweets$logRetweets), max(pos_tweets$logRetweets))

#Visualize dist. of log(Retweets) for Negative tweets:
#ggdensity(neg_tweets$logRetweets,
#          main = "Google Negative: log(Retweets)",
#          xlab = "log(Retweets)") + xlim(min(neg_tweets$logRetweets), max(pos_tweets$logRetweets))
...
#Perform Mann Whitney U test on neutral vs positive tweets (for retweets):
res = wilcox.test(neut_tweets$`Number of Retweets`, pos_tweets$`Number of Retweets`)
res
...
#nneut = nrow(neut_tweets)
#nneg = nrow(neg_tweets)
df = mkt2(neut_tweets$`Number of Retweets`, pos_tweets$`Number of Retweets`)
#df = tibble(gr = factor(c(rep("Positive", npos), rep("Neutral", nneut), rep("Negative", nneg))),
...
df = mkt2(neut_tweets$logRetweets, pos_tweets$logRetweets)
sf = shifthd_pbci(data = df, formula = obs ~gr, doall = TRUE)
#Add more colors here, rather than below:
palette1 = c("violetred2", "steelblue2", "springgreen1", "slategray3")
