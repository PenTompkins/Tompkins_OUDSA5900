# -*- coding: utf-8 -*-
#This file is being created to perform the 'Dynamic_Count.py' process, but with a step to lemmatize words
#Otherwise, this file should be exactly the same
#!Note: Just as 'Dynamic_Count.py' has been retired in favor of 'Dynamic_CountAll.py', this file has been retired in favor of 'Dynamic_CountAll2.py'
#Ultimately, no analysis was run through this file
#
#Overall, this is program: 30
...
#!!Trying to determine best position in file to perform lemmatization:
...
#test = lem_with_postag(company_tweets, "Content")
#print("Test")
#print(test)
...
if propOT >= minThresh and propIRT >= minThresh:
    ...
    #For this version, lemmatize the official tweets:
    official_tweets2["Content"] = lem_with_postag(official_tweets2, "Content")
    ...
    #For this version, lemmatize the IRT tweets:
    IRT_tweets2["Content"] = lem_with_postag(IRT_tweets2, "Content")
    ...
    plt.xlabel('Tweet Word Count')
    plt.ylabel('Number of Tweets')
    plt.hist(tweet_lengths_mid_IRT)
    plt.show()
...
#In the case that there aren't enough official tweets for analysis:
    ...
    print("%s doesn't seem to have enough official tweets for analysis. IRT tweets will be used only" % company_name)
    ...
    #For this version, lemmatize the tweets:
    IRT_tweets2["Content"] = lem_with_postag(IRT_tweets2, "Content")
    ...
    ##Perform same analysis over top 25% of all tweets: (In this case, will mostly be IRT tweets)
    #Specify the number of rows to take as the top 25% of IRT tweets:
    nr = int(round(0.25 * len(IRT_clean)))
    ...
    #Examine n-grams for top 25% of all tweets:
    bigrams_IRT25 = (pd.Series(nltk.ngrams(filtered_toks7, 2)).value_counts())[:20]
    ...
    plt.rc('ytick', labelsize=SMALL_SIZE)
    bigrams_IRT25.sort_values().plot.barh(color='blue', width=0.9, figsize=(12,8))
    plt.title("20 Most Common %s Top 25 Pct Tweet Bi-grams" % company_name)
    plt.ylabel("Bi-gram")
    plt.xlabel("Number of Occurences")
    plt.show()
    ...
    #Visualize tri-grams:
    trigrams_IRT25.sort_values().plot.barh(color='blue', width=0.9, figsize=(12,8))
    plt.title("20 Most Common %s Top 25 Pct Tweet Tri-grams" % company_name)
    plt.ylabel("Tri-gram")
    plt.xlabel("Number of Occurences")
    plt.show() 
    
    
    ##Perform same analysis over median 50% of all tweets: (in this case, vast majority of tweets are IRT)
...
#In case there aren't enough IRT tweets for analysis:
    ...
    print("%s doesn't seem to have enough IRT tweets for analysis" % company_name)
    ...
    ##Perform same analysis over top 25% of company tweets only: (in this case, most tweets will be official)
    #Specify the number of rows to take as the top 25% of tweets:
    nr = int(round(0.25 * len(official_clean)))
    
    #Sort the data based on number of likes:
    official_clean = official_clean.sort_values(by = "Number of Likes", ascending=False)    
    
    #Extract the top 25% of tweets from the sorted data:
    top_official = official_clean.head(nr).copy()
    ...
    ##Perform same analysis over median 50% of company tweets: (in this case, mostly official tweets)
    #Grab the median 50% of company tweets:
    print("Across all company data, there are %i tweets" % len(official_clean))
    print("In the top 25 pct there are %i tweets" % len(top_official))
    desired_num = int(round((0.5 * len(official_clean))))
    print("We should get %i tweets for middle 50 pct" % desired_num)
    nr2 = nr + 1
    ...
    ##Perform same analysis over bottom 25% of tweets: (in this case, vast majority of tweets are official)
    #Grab the bottom 25% performing official tweets
    bottom_official = official_clean.tail(nr).copy()
    #print(bottom_official.shape)
