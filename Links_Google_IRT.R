#This file is being created to run link effect val. process over Google IRT tweets
#
#Overall, this is program: 74...
...
data = read_excel("C:/Users/Pendl/OneDrive/Documents/TwitterProject/OutlierFree_data/Google_outsRemoved.xlsx")
...
ggdensity(link_tweets$`Number of Likes`,
          main = "Google Linked: Likes",
          xlab = "Number of Likes") #+ xlim(0, max(noLink_tweets$`Number of Likes`))

#Visualize dist. of likes for noLink_tweets:
ggdensity(noLink_tweets$`Number of Likes`,
          main = "Google No Link: Likes",
...
ggdensity(noLink_tweets$logLikes,
          main = "Google No Link: log(Likes)",
          xlab = "log(Likes)") + xlim(min(noLink_tweets$logLikes), max(link_tweets$logLikes))
...
ggdensity(link_tweets$logRetweets,
          main = "Google Linked: log(RTs)",
          xlab = "log(Retweets)") + xlim(min(noLink_tweets$logRetweets), max(link_tweets$logRetweets))

#Visualize dist. of log(retweets) for noLink_tweets:
ggdensity(noLink_tweets$logRetweets,
          main = "Google No Link: log(RTs)",
          xlab = "log(Retweets)")
